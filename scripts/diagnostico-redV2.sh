...
# ── 2.5. Gateway predeterminado ───────────────────────────
log "--- 2.5. CONECTIVIDAD CON GATEWAY ---"

gateway=$(ip route | awk '/default/ {print $3; exit}')

if [ -n "$gateway" ]; then
    log "Gateway detectado: $gateway"

    if ping -c 2 -W 3 "$gateway" &>/dev/null; then
        ok "Gateway $gateway — alcanzable"
    else
        fail "Gateway $gateway — no responde a ping"
    fi
else
    warn "No se encontró un gateway predeterminado"
fi
